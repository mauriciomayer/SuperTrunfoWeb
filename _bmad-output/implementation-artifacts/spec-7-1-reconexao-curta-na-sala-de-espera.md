...
- `frontend/src/screens/SalaDeEspera.tsx`, `frontend/src/App.tsx` -- NENHUMA mudança nesta história (ver Limites e Restrições "Nunca"); Story 7.2 (separada) cobre o aviso visual quando a reconexão falha.
...
- [x] `backend/src/rooms/PartidaRoom.integration.test.ts` -- os 3 testes do Mapa de Código, usando `client.leave(false)` (desconexão NÃO consentida — fecha a conexão sem enviar `LEAVE_ROOM`) e `testServer.sdk.reconnect(reconnectionToken)` (reconexão real, mesmo `sessionId`).
...
- [x] `backend/src/rooms/PartidaRoom.integration.test.ts` -- os 3 testes novos do Mapa de Código (guard de `iniciarPartida`; único cliente real cai e a sala sobrevive; assento reservado não é ocupado por terceiro).
...
**Critérios de Aceite:**
- Dado uma Sala de Espera com `estado === "AguardandoJogadores"`, quando a conexão de um Jogador cai abruptamente e reconecta dentro de 30s, então ele retoma o MESMO assento (`sessionId` preservado), sem nunca ter saído de `state.jogadores` (FR-36).
- Dado a mesma situação, quando a janela de 30s expira sem reconexão, então o assento é removido de `state.jogadores` exatamente como o comportamento já existente.
- Dado uma Partida já em andamento (`estado` != `"AguardandoJogadores"`), quando um Jogador humano perde a conexão abruptamente, então o comportamento confirmado da História 3.2 continua idêntico (assento vira IA, sem nenhuma janela de reconexão) — provado via os testes já existentes dessa história continuando a passar sem alteração.
- Dado um assento no meio da janela de reconexão, quando o host clica em "Iniciar", então o servidor rejeita o pedido e a Partida não começa (achado da revisão).
- Dado uma Sala de Espera com um único cliente real conectado, quando esse cliente cai abruptamente, então a sala NÃO é destruída durante a janela de tolerância (achado da revisão -- cenário exato do incidente original).
...
- **Emenda:** Limites e Restrições "Sempre"/Matriz de Entrada-Saída (dentro do `<frozen-after-approval>`) ganharam o novo comportamento; Mapa de Código/Tarefas (fora do frozen) ganharam o mecanismo de rastreamento (`sessoesReconectando`) e o novo guard, mais 3 testes novos e 2 correções cosméticas também levantadas na mesma revisão.
...
**Comandos:**
- `cd backend && npx vitest run PartidaRoom.integration` -- esperado: todos os testes passam, incluindo os 3 novos, sem nenhuma regressão nos testes de `onLeave`/Story 3.2 já existentes.
