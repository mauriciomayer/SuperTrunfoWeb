...
| Conexão perdida | Mesa de Jogo | Um bot assume o assento do Jogador desconectado, seguindo o Monte e o estado dele exatamente de onde parou — a Partida continua sem travar para os demais. `[NOTE FOR PM]` Isso estende o comportamento de preenchimento por IA do PRD (FR-5, hoje só cobre vagas vazias antes do início) para também cobrir desconexão em andamento — vale confirmar/registrar no PRD antes da Arquitetura. |
| Reconexão | Mesa de Jogo | `[ASSUMPTION]` Não discutido na Discovery — decisão de design proposta: se o Jogador original reabrir o link durante a mesma Partida, o controle do assento volta pra ele **no início da próxima Rodada** (nunca no meio de uma Rodada em andamento, pra não interromper uma jogada da IA em curso). Até lá, ele entra como espectador do que a IA está fazendo no lugar dele. Vale confirmar com o usuário ou deixar para a Arquitetura decidir o mecanismo exato. |
...
5. `[ASSUMPTION]` **Reconexão pós-desconexão** — a regra proposta (controle volta pro humano só no início da próxima Rodada, nunca no meio de uma em andamento) é uma decisão de design deste documento; o PRD (FR-23) registra a mesma pendência como Questão em Aberto §8.6, ainda não confirmada pelo usuário.
